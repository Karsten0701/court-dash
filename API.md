...
org is a private tenant with its own users, games, leaderboard, and history ÔÇö data never
crosses org boundaries.

There are three roles across two worlds:

- **`user`** ÔÇö plays inside exactly one org.
- **`manager`** ÔÇö runs one org. Has the powers a single-app `admin` used to have, but
  scoped to their own org, plus they own the org's branding (name + accent color). An org
  can have multiple managers.
- **`admin`** ÔÇö a **platform** operator that belongs to **no** org. Manages orgs from
...
gameplay requests** ÔÇö the server scopes everything to the caller's org automatically.
...
| ----------------- | ------------------- | --------- |
| `pending_payment` | ÔØî | Just created, never paid. Activate via the public payment simulation. |
| `active`          | Ô£à | Normal ÔÇö full read & write. |
| `inactive`        | Ô£à (read-only) | Deactivated. Members can log in and **read** everything, but **all writes are blocked** until the org is reactivated (manager payment or admin override). |
...
> Members of an `inactive` org **can** log in ÔÇö they get a read-only session (see
...
| ----------------- | ------ | -------- | ---------------------------------------- |
| `name`            | string | Ô£à       | Display name of the org                  |
| `accentColor`     | string | Ô£à       | Hex color for branding, e.g. `#1E88E5`   |
| `managerEmail`    | string | Ô£à       | Email for the first `manager` account    |
| `managerPassword` | string | Ô£à       | Password for the first `manager` account |
...
| --------- | ------ | -------- | ------------------------ |
| `q`       | string | Ô£à       | Partial org name to match |
...
| Parameter | Type    | Required | Description                                              |
| --------- | ------- | -------- | -------------------------------------------------------- |
| `orgId`   | integer | ÔÜá´©Å       | Required when unauthenticated; ignored when logged in (caller's org is used) |
...
| Field        | Type    | Required | Description                   |
| ------------ | ------- | -------- | ----------------------------- |
| `roundNumber` | integer | Ô£à       | Round number to set (ÔëÑ 1)     |
...
| Field                 | Type    | Required | Description                        |
| --------------------- | ------- | -------- | ---------------------------------- |
| `requestedForUserId`   | integer | Ô£à       | User ID of the player being invited |
| `message`              | string  | ÔØî       | Optional message with the request  |
...
| Field    | Type   | Required | Description              |
| -------- | ------ | -------- | ------------------------ |
| `status` | string | Ô£à       | Either `accepted` or `rejected` |
...
| Field      | Type    | Required | Description                         |
| ---------- | ------- | -------- | ----------------------------------- |
| `winnerId` | integer | Ô£à       | `userID` of the winning participant |
| `scores`   | array   | Ô£à       | Final score for every participant   |
